...
export class PermitsScraper extends BaseScraper {
  ...
  async scrape(location: string): Promise<ScrapedLead[]> {
    ...
    try {
      ...
      // Real scraping implementation only - no mock data
      const permitUrls = this.getPermitUrls(location);

      if (permitUrls.length === 0) {
        this.log(`No permit database URLs configured for ${location} - data not available`);
        return leads;
      }
      ...
      if (leads.length === 0) {
        this.log(`No permit leads found in ${location} - real data returned empty`);
      } else {
        this.log(`Found ${leads.length} real permit leads in ${location}`);
      }
    } catch (error: any) {
